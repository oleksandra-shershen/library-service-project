...
class PaymentSchema:
    ...
    payment_process_schema = extend_schema(
        request={
            'application/json': {
        ...
        responses={
            303: OpenApiParameter(
                ...
                type={'type': 'object', 'properties': {'url': {'type': 'string'}}},
                required=['url']
            ),
            400: OpenApiParameter(
                description='Error message',
                type={'type': 'object', 'properties': {'error': {'type': 'string'}}},
                required=['error']
            ),
        },
        description="Create a Stripe payment session for a given borrowing ID and return the URL for the payment."
    ),

    payment_completed_schema = extend_schema(
        parameters=[
            OpenApiParameter(
                name="session_id",
        ...
        responses={
            200: OpenApiParameter(
                ...
                type={'type': 'object', 'properties': {'message': {'type': 'string'}}},
                required=['message']
            ),
            400: OpenApiParameter(
                description='Error message',
        ...
        description="Handle the completion of a payment session. Update the payment status to 'PAID' if the session ID is valid."
    ),
